...
const RaiseFunds = () => {
  ...
  const updateForm = async (form) => {
    ...
    if (stage === 4) { //Submit the data to the server
      ...
      formData.append('owner', JSON.parse(localStorage.getItem('Login')).record.id);
  ...
  if (localStorage.getItem('Login') == null) return <div>Please Login First</div>
  return (
...
const MessageComponent = ({ stage }) => {
  ...
  // const click = async () => {
  //   const pb = new PocketBase('http://127.0.0.1:8090');
  //   const record = await pb.collection('fundraisers').getOne('t9iq6h0r41v6ztb');
  //   console.log({ record })
  //   const url = pb.files.getUrl(record, record.thumbnail);
  //   setImg(url)
  // }
  // useEffect(() => {
  //   click();
  ...
  return (
    <>
      {/* <img src={img} className="w-4/6 h-4/6 rounded-full" /> */}
