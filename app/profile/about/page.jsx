    ...
    <>
      <div className="group grid place-items-center rounded-xl p-10 shadow-lg">
        ...
        <div className="h-full w-full grid gap-4">
          <Field name="Name" detail={loginData.record?.name} field1="New Name" field2={"Enter Password"}
                 placeholder={"Full Name"} />
          <Field name="Password" detail={"•••••••••"} field1={"New Password"} field2={"Current Password"}
                 placeholder={"•••••••••"} type={"password"} />
          <Field name="Email" detail={loginData.record?.email} field1={"New Email"} field2={"Enter Password"}
                 placeholder={"name@example.com"} />
          <Field name="Username" detail={loginData.record?.username} field1={"New Username"} field2={"Enter Password"}
  ...
  return (
    <>
      <div className={"w-full h-full flex flex-col p-6"}>
        ...
        <div className="mb-6"><label className="block mb-2 text-sm font-medium text-gray-900">
          {props.field2}</label>
        ...
        <div>
          <label className="block mb-2 text-sm font-medium text-gray-900">  {props.field1}</label>
          <input type={props.type ? props.type : "text"} ref={ifRef}
                 className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                 placeholder={`${props.placeholder}`} />
        </div>
        <div className={"grid grid-rows-1 grid-cols-2 w-full gap-4 flex-row justify-around pt-6 pb-3"}>
          <Button type="septenary" text="Change" className="flex w-full md:w-auto" onClick={async () => {
            ...
            try {
              ...
              switch (props.name.toLowerCase()) {
                ...
                case "email":
                  let emailExists = await pb.collection("users").getList(1, 50, { filter: `email = '${ifRef.current.value}'` });
                ...
                case "username":
                  let usernameExists = await pb.collection("users").getList(1, 50, { filter: `username = '${ifRef.current.value}'` });
